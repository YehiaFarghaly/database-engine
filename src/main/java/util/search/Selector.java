...
public class Selector {
	...
	private static Object[] getMax(String tableName, String[] col) {
		...
		for (String[] arr : columns) {
			if (arr[Constants.COLUMN_NAME_INDEX].equals(col[0])) {
				colMax[0] = TypeParser.typeParser(arr[Constants.COL_MAX_VALUE_INDEX], arr[Constants.COLUMN_TYPE_INDEX]);
			} else if (arr[Constants.COLUMN_NAME_INDEX].equals(col[1])) {
				colMax[1] = TypeParser.typeParser(arr[Constants.COL_MAX_VALUE_INDEX], arr[Constants.COLUMN_TYPE_INDEX]);
			} else if (arr[Constants.COLUMN_NAME_INDEX].equals(col[2])) {
				colMax[2] = TypeParser.typeParser(arr[Constants.COL_MAX_VALUE_INDEX], arr[Constants.COLUMN_TYPE_INDEX]);
			}
		}
		return colMax;
	}

	private static Object[] getMin(String tableName, String[] col) {
		Object colMin[] = new Object[3];
		CsvReader reader = new CsvReader();
		List<String[]> columns = reader.readTable(tableName);
		for (String[] arr : columns) {
			if (arr[Constants.COLUMN_NAME_INDEX].equals(col[0])) {
				colMin[0] = TypeParser.typeParser(arr[Constants.COL_MIN_VALUE_INDEX], arr[Constants.COLUMN_TYPE_INDEX]);
			} else if (arr[Constants.COLUMN_NAME_INDEX].equals(col[1])) {
				colMin[1] = TypeParser.typeParser(arr[Constants.COL_MIN_VALUE_INDEX], arr[Constants.COLUMN_TYPE_INDEX]);
			} else if (arr[Constants.COLUMN_NAME_INDEX].equals(col[2])) {
				colMin[2] = TypeParser.typeParser(arr[Constants.COL_MIN_VALUE_INDEX], arr[Constants.COLUMN_TYPE_INDEX]);
			}
		}
		return colMin;
	...
	public static int [] getPagesNumbers(List pagespathes, Table table) {
		int [] res = new int[pagespathes.size()]; 
		for(int i=0;i<pagespathes.size();i++) {
			res[i] = table.getPageIdxFromPath(pagespathes.get(i).toString());
		}
		return res;
	}
	

	public static Vector<Tuple> selectWithIndex(OctreeIndex index, SQLTerm[] arrSQLTerms, String[] ColumnsNames, Table table) throws DBAppException {
...
//    }
	...
	public static Vector<Tuple> selectFromTableWithIndex(String strTableName, Hashtable<String, Object> colNameValue,
			String operator, int[] pageIndex) throws DBAppException {
		Vector<Tuple> result = new Vector<>();
